...
class ElfFileComponent(IComponent):
    ...
    def _parse_elf_file(self):
        if self.objdump_args is not None:
            self.elf_info = self._do_objdump()
        else:
            self.elf_info = scan_ie_elf(self.value, self.module_entry, self.markers)
            # it was like that in IBST 1.0 - no idea why:
            self.elf_info.ie.text_start = self.elf_info.entry_address
    ...
    def validate(self):
        super().validate()
        if not self.value:
            self.error_message = "Path to the file was not specified"
            return
        try:
