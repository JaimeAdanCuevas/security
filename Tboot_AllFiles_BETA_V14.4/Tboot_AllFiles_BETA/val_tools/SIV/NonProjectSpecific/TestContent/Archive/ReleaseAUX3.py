...
# $Revision$
############################################################################
#This script Releases AUX3 index in TPM
...
def main():
    ...
    if(os.system("tpmnv_getcap | grep 0x50000003")!=0):
        print "AUX3 index already released...."
    else:
    	TpmnvGetcapValue=subprocess.Popen(["tpmnv_getcap"],stdout=subprocess.PIPE)
    	tpmnv= TpmnvGetcapValue.stdout.read()
    	if Aux3Index in tpmnv:
    		release=os.system("tpmnv_relindex -i 0x50000003 -p ownerauth")
        	if release == 0:
			print ' Successfully released AUX3.......'
        	else:
			print ' Failed releasing AUX3.......'
                	return 0
