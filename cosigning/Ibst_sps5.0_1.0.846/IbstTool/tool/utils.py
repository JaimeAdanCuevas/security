...
#
def calculate_hash(buffer, hash_type=ss.ShaType.SHA384):
    if hash_type == ss.ShaType.SHA256:
        return hashlib.sha256(buffer).digest()
    elif hash_type == ss.ShaType.SHA384:
        return hashlib.sha384(buffer).digest()
...
def process_private_key(data, hash_type):
    ...
    if isinstance(pkey, rsa.RSAPrivateKey):
        return process_private_rsa_key(pkey, hash_type)
    if isinstance(pkey, ec.EllipticCurvePrivateKey):
        return process_private_ec_key(pkey, hash_type)


def process_private_rsa_key(pkey, hash_type):
    signing_key = ss.RsaSigningKey()
...
def process_private_ec_key(pkey, hash_type):
...
def process_public_key(data, hash_type):
    ...
    if isinstance(pkey, rsa.RSAPublicKey):
        return process_public_rsa_key(pkey, hash_type)
    if isinstance(pkey, ec.EllipticCurvePublicKey):
        return process_public_ec_key(data, hash_type)


def process_public_rsa_key(pkey, hash_type):
...
def process_public_ec_key(pkey, hash_type):
...
def hashed_key_printer(signing_key, file_name):
    ...
    if isinstance(signing_key, bytes):
        txt_hashed_key = Converter.bytes_to_string(calculate_hash(signing_key, ss.ShaType.SHA256))
        key_kind = 'AesKey'
...
def validate_path_to_file(component, path: string):
    ...
    if (directory_path and not os.path.isdir(directory_path)) or len(file_name) > 255:
        raise ComponentException("Invalid path to file: " + path, component.name)


def check_file_path(path, can_be_empty=True, throwing=True):
