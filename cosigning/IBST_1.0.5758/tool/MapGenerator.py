...
class XmlMapFormatter(IMapFormatter):
    mapExt = '.xml'
    attributesDefault = [IComponent.Tags.OFFSET, IComponent.Tags.SIZE]
    skipEmpty = True
    maxLevel = 1
    ...
    attributesMap = {BitFieldComponent.Bit: [IComponent.Tags.VALUE]}
    maxValueSize = 128
    ...
    def _create_xml_entry(self, component: IComponent, parent_node, indent=0):
        ...
        element_name = component.map_name if component.map_name else component.name

        if parent_node is None:
            element = Element(component.name)
        elif (self.maxLevel is not None and indent < self.maxLevel) or component.map_name:
            element = SubElement(parent_node, element_name)
        else:
            element = SubElement(parent_node, component.name)

        if IComponent.Tags.OFFSET in attributes:
            element.attrib[component.Tags.OFFSET] = hex(component.offset)
        if IComponent.Tags.SIZE in attributes:
            element.attrib[component.Tags.SIZE] = hex(component.size)

        if component.encryption_key_component:
            if IComponent.Tags.VALUE in attributes:
                element.attrib[component.Tags.VALUE] = '(encrypted)'
        elif component.children:
            if self.maxLevel is None or indent < self.maxLevel:
                for child in component.children:
                    if child.is_enabled() and (not self.skipEmpty or child.size > 0):
                        self._create_xml_entry(child, element, indent + 1)
            elif component.has_map_name_descendant():
                for child in component.children:
                    if (child.map_name or child.has_map_name_descendant()) and child.is_enabled() and \
                            (not self.skipEmpty or child.size > 0):
                        self._create_xml_entry(child, element, indent + 1)
        elif IComponent.Tags.VALUE in attributes:
            element.attrib[component.Tags.VALUE] = self._format_value(component)
        return element
...
class XmlInfoFormatter(XmlMapFormatter):
    attributesDefault = [IComponent.Tags.OFFSET, IComponent.Tags.SIZE, IComponent.Tags.VALUE]
    maxLevel = None
    skipEmpty = False
...
class MapGenerator:
    ...
    def generate_map(self, map_root: IComponent, map_path, ext=''):
        ...
        FileManager.save_text_file(final_path, formatter_inst.create_map(map_root))
        return final_path
