...
def parseCommandLine():
    ...
    parser.add_option("--profile_name", action="store",
                      dest="ProfileString", default="Logging",
                      help="Indicates the PuTTY profile name to load for logging.  Default is 'Logging'")
...
def main():
    ...
    #  Start up new instance of PuTTY
    _sys.path.append("C:\Users\Public\Desktop\ ")
    sDescription    = "start new instance of PuTTY with profile: '%s'" % oCmdlineOptions.ProfileString
    sCommand        = "notepad.exe" 
    sCommand        = "start \"PuTTY Start Window\" \"C:/Users/Public/Desktop/putty.exe\" -load %s" % oCmdlineOptions.ProfileString
...
#                                                       bDoNotRun=bDebug)
    ...
    #----
    lLogger.info("Starting command to %s" % sDescription)
    try:
        nReturnCode = _subprocess.call(sCommand, shell=True)
        if nReturnCode < 0:
            lLogger.error("Command was terminated by signal '%d'" % -nReturnCode)
            bErrorsOccurred = True
        elif nReturnCode > 0:
            lLogger.error("Command exited with error status '%d'" % nReturnCode)
            bErrorsOccurred = True
        else:
            lLogger.info("Command successfully returned with status '%d'" % nReturnCode)
    except OSError as e:
        lLogger.error("Execution of command failed.  Return code: '%d'" % -nReturnCode)
        lLogger.error("  Exception returned was %s" % e)
        bErrorsOccurred = True
    #----
