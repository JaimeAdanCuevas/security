...
#+----------------------------------------------------------------------------+
#| $Id: LogPcr.py 136 2015-02-03 00:52:09Z amr\egross $
...
"""
    Finds the PCRS file in /sys/devices and copies it to the current dir
"""
...
#+----------------------------------------------------------------------------+
#|  Make a copy of the PCRs file in the local directory
#|
#|  Inputs:     None
...
#+----------------------------------------------------------------------------+
def logPcrs():
    sCommand = "cp `find /sys/devices/ -name pcrs` ./pcrs.old"

    lLogger.info('Attempting to logging PCR values...........')

    if( _os.system(sCommand) == 0 ):
        lLogger.info('    Successfully made a copy of current PCR values')
        return True
    else:
        lLogger.error('    Failed to make a copy of current PCR values')
        return False
