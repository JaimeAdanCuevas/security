...
# $Revision$
############################################################################
#This script releases AUX2 index in TPM
...
def main():
    ...
    if(os.system("tpmnv_getcap | grep 0x50000002")!=0):
        print "AUX2 index already released...."
    else:
    	TpmnvGetcapValue=subprocess.Popen(["tpmnv_getcap"],stdout=subprocess.PIPE)
    	tpmnv= TpmnvGetcapValue.stdout.read()
    	if Aux2Index in tpmnv:
    		release=os.system("tpmnv_relindex -i 0x50000002 -p ownerauth")
    		if release == 0:
			print ' Successfully Released AUX2.......'
    		else:
			print ' Failed releasing AUX2.......'
			return 0
    LogDelimiter()
    logger.info("Pass")
    LogDelimiter()
