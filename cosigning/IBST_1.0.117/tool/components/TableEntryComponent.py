...
class TableEntryComponent(IComponent):
    ...
    def build_layout(self, buffer, clear_build_settings=False):
        self.set_size(0)
        table = self.calculate_value_from_path(self.table_path)
        self.entry = self._find_table_entry(table)
        if self.entry:
    ...
    def _find_table_entry(self, table):
        ...
        for entry in table.children:
            ...
            if type(result) is not bool:
                raise ComponentException(f"Key should be bool type, now is {type(result)}", self.name)
            if result:
                return entry
        if self.required:
