...
class ByteArrayComponent(IComponent):
    ...
    def __init__(self, xml_node, **kwargs):
        ...
        self.align_byte = LibConfig.byteArrayPaddingValue
    ...
    def get_default_value(self):
        return self.align_byte * self.data_size
        ...
        if isinstance(value, int):
            if self.data_size is None:
                raise ComponentException(
                    ...
                    self.name)
            super()._set_value(value.to_bytes(self.data_size, 'little'))
        else:
            super()._set_value(value)

    def get_value_string(self):
        if self.value:
            return Converter.bytes_to_string(self.value)
        return ''

    def _is_empty(self):
        return all(digit == 0 for digit in self.value)
